...
namespace WindowsFormsApp8
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (comboBox2.SelectedIndex == 0)
            {
                if (Program.f)
                {
                    ...
                    {
                        MessageBox.Show("Возможно, выбранного предмета у заданной группы нет", "Ошибка");
                    }
                }
                else
                {
            ...
            else if (comboBox2.SelectedIndex == 2)
            {
                if (Program.f)
                {
                    ...
                    {
                        MessageBox.Show("Возможно, выбранного предмета нет у данного студента", "Ошибка");
                    }
                }
                else
                {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
                ...
                try
                {
                    command = new SqlCommand("SELECT COUNT(Mark) FROM Marks WHERE Mark = @p4 AND Student_id =@p1 AND Subject_id = @p2 AND Datatime =@p3", sqlcon);
                    command.Parameters.AddWithValue("p1", comboBox6.Text);
                    command.Parameters.AddWithValue("p2", comboBox7.Text);
                    command.Parameters.AddWithValue("p3", maskedTextBox1.Text);
                    command.Parameters.AddWithValue("p4", textBox6.Text);
                    command.ExecuteNonQuery();
                    int cou = 0;
                    cou = (int)command.ExecuteScalar();
                    if (cou != 0)
                    {
                    ...
                    {
                        MessageBox.Show("Такого значения нет в базе данных", "Ошибка");
                    }
                }
                catch
                {
